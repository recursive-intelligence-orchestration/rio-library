...
- in this deployment, the script is mounted inside the container at
  `/a0/instruments/default/main/rio-library/create_agent.sh` via the
...
- **Where/how to run**
  ...
  - `cd /a0/instruments/default/main/rio-library` (or rediscover the
...
- **After running**
  ...
  - To locate the new agent's `behaviour.md`, apply the **Config-First
    ...
    See [RIO_LIBRARY_AGENT_REASONING.md → Config-First Rule](layers/common/knowledge/default/main/rio-library/RIO_LIBRARY_AGENT_REASONING.md#config-first-rule).
...
```json
{
  ...
  "tool_args": {
    "runtime": "terminal",
    "session": 0,
    "code": "cd /a0/instruments/default/main/rio-library && ./create_agent.sh /containers/a0-template /containers/a0-myagent dest_display=\"My Agent\" port_base=500 knowledge_dir=tbc no_docker=true"
  }
}
```
...
cd /a0/instruments/default/main/rio-library && ./create_agent.sh a0-template a0-myagent
...
```bash
# Host shell (outside any Agent Zero container), from rio-library repo root:
./create_agent.sh a0-template a0-myagent
```
